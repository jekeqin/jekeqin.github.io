// https://gitee.com/chenbont/barcode-code128/
...
export function code128(text, canvas) {
    var uw = 3;
    var uh = uw * 20;
    if( !canvas )
        canvas = document.createElement('canvas');

    var elements = createElement(text)
    
    var units = elements.map(function (item) {
        return item[5];
    }).join('').split('');
    canvas.height = uh;
    ...
    for (var i = 0, len = units.length; i < len; i++) {
        if (+units[i]) {
            drawUnit(gl, i, uw, uh, '#000');
        } else {
            drawUnit(gl, i, uw, uh, '#fff');
        }
    }
    gl.save();
    ...
    return {
        ...
        data:gl.getImageData(0, 0, canvas.width, canvas.height), 
        canvas:canvas
    }
}
